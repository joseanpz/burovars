...
from buro.models import VariablesBuroCuentasModel


@dataclass(unsafe_hash=True, order=True)
class VariablesVectorFechaCuentas:
    fecha: date
...
class ConstructorVariablesAgregadasCuentas:
    ...
    # middle vars
    vectores_fecha: List[VariablesVectorFechaCuentas] = field(init=False)
    lista_var_cuentas: List[dict] = field(init=False)
    lista_var_cuentas_ingresos: List[VariablesVectorFechaCuentas] = field(init=False)
    ...
    def clase_subvar_vec(self):
        return VariablesVectorFechaCuentas

    def __post_init__(self):
        self.configura_vectores_fecha()
    ...
    def configura_vectores_fecha(self):
        self.vectores_fecha = []
        for var_buro_cuenta in self.lista_var_buro_cuentas:
            self.vectores_fecha.append(
                self.clase_subvar_vec(var_buro_cuenta.cuf_fec_ape,
                                      self.vector_fecha_limite(var_buro_cuenta.cuf_fec_ape))
            )
            self.vectores_fecha.append(
                self.clase_subvar_vec(var_buro_cuenta.cuf_fec_cie,
                                      self.vector_fecha_limite(var_buro_cuenta.cuf_fec_cie))
                if var_buro_cuenta.cuf_fec_cie
                else self.clase_subvar_vec(var_buro_cuenta.fecha_alta,
                                           self.vector_fecha_limite(var_buro_cuenta.fecha_alta))
            )
    ...
    def construye_variables_agregadas_cuentas(self, campo_agr: CampoAgregado):
        if isinstance(campo_agr.campo, CampoAgregado):
            ...
            filtro = list(
                filter(lambda x: getattr(x.var_cuentas_ingreso, campo_agr.campo.nombre, None) is not None, lista))
            filtro.sort()
